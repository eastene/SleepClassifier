...
class DataPrepper:
    ...
    def mat2npy(self, files, column="epochs", struct_layers=4, channel_idx=2, label_idx=5):
        ...
        print("Writing to npz...0.0%", end="\r")
        for i, f in enumerate(files):
            ...
            for j in range(n_rows):
                ...
                np.savez(out_str, x=np.squeeze(data[j][0][0][0][channel_idx]), y=data[j][0][0][0][label_idx] % 6)
            #     labels.append(data[j][0][0][0][label_idx])
            # np.save(os.path.join(sample_dir, 'labels.npz'), np.array(labels))
            print("Writing to npz...{:3.1f}%".format(i / len(files) * 100), end="\r")

        print("Writing to npz...100%")
        print("Done.")
